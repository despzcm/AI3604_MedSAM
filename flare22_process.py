...
nii_path = "./data/FLARE22Train/FLARE22Train/images"  # path to the nii images
gt_path = "./data/FLARE22Train/FLARE22Train/labels"  # path to the ground truth
...
for name in tqdm(names[38:39]):  # use the remaining 10 cases for validation
    ...
    if len(z_index) > 0:
        ...
        # Save the images and ground truth as npy files
        for i in range(img_roi.shape[0]):
            img_i = img_roi[i, :, :]
            img_3c = np.repeat(img_i[:, :, None], 3, axis=-1)
            
            # Using OpenCV for resizing (much faster)
            resize_img_skimg = cv2.resize(img_3c, (image_size, image_size), interpolation=cv2.INTER_CUBIC)
            ...
            # Resize ground truth with nearest-neighbor interpolation (faster)
            resize_gt_skimg = cv2.resize(gt_i, (image_size, image_size), interpolation=cv2.INTER_NEAREST)
            ...
            img_3c = np.repeat(img_i[:, :, None], 3, axis=-1)
            print("begin resize")
            resize_img_skimg_o = transform.resize(
                img_3c,
            ...
            print("mid2")
            resize_img_skimg_01_o = (resize_img_skimg - resize_img_skimg.min()) / np.clip(
                resize_img_skimg.max() - resize_img_skimg.min(), a_min=1e-8, a_max=None
            )  # normalize to [0, 1], (H, W, 3)
            print("mid")
            gt_i = gt_roi[i, :, :]
            resize_gt_skimg_o = transform.resize(
                gt_i,
                (image_size, image_size),
                order=0,
                preserve_range=True,
            ...
            resize_gt_skimg_o = np.uint8(resize_gt_skimg)
            print(np.all(operator.eq(resize_img_skimg_01, resize_img_skimg_01_o)))
            print(np.all(operator.eq(resize_gt_skimg, resize_gt_skimg_o)))
            '''
            
            # Ensure dimensions match
            assert resize_img_skimg_01.shape[:2] == resize_gt_skimg.shape
            
            # Save the resized image and ground truth
            # print(1111111111111)
            np.save(
                join(
                    npy_path,
